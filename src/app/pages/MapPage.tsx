...
function readStoredPinsByMarket(marketId: MarketId): Map<string, DraftStorePin> {
  try {
    const raw = window.localStorage.getItem(OWNER_DASHBOARD_STORAGE_KEY);
    if (!raw) return new Map<string, DraftStorePin>();
    ...
    return new Map(
      stores
        ...
        .map((store): [string, DraftStorePin] => [store.name, store]),
    );
  } catch {
    return new Map<string, DraftStorePin>();
  }
}

function readPinsFromStoresByMarket(stores: DraftStorePin[], marketId: MarketId): Map<string, DraftStorePin> {
  return new Map(
    stores
      .filter(
        (store) =>
...
export function MapPage() {
  ...
  const allStores = useMemo(() => {
    const overrides = sharedStores
      ? readPinsFromStoresByMarket(sharedStores, selectedMarket)
      : readStoredPinsByMarket(selectedMarket);
    return STORES_BY_MARKET[selectedMarket].map((store) => {
      const override = overrides.get(store.name);
