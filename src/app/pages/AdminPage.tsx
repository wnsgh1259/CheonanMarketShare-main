...
export function AdminPage() {
  ...
  const dummyStores = useMemo<DraftStore[]>(() => {
    const marketOrder: MarketId[] = ["jungang", "byeongcheon", "seonghwan"];
    return marketOrder.flatMap((marketId, marketIndex) =>
  ...
  const mergedStores = useMemo(() => {
    const byKey = new Map<string, DraftStore>();
    dummyStores.forEach((store) => {
      byKey.set(`${store.marketId}-${store.name}`, store);
    });
    draft.stores.forEach((store) => {
      const key = `${store.marketId}-${store.name}`;
